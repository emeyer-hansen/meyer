---
title: "meyer"
subtitle: "Novel & Classical Statistical Tools"
author: "Emil Niclas Meyer-Hansen"
description: "Provides novel functions for rescaling vectors as correlation coefficients and qualitatively determining degrees of freedom (df) for a prior Student t-distribution, including simple functions useful for frequentist and Bayesian statisticians."
institute: ""
date: "5/12/2023"
abstract: "YAML"
keywords: 
...
The novel procedure of *meyerization* involves rescaling a numeric vector *x* similar to that of a *correlation coefficient*, which readers are likely most familiar with in the form of Pearson's *r*, characterized by a range of [-1; 1]. By rescaling data in this manner, coefficients from linear regressions become analogous to - and interpretable as - a correlation coefficient. 

I term this *meyerization* in a similar manner to Charles P. Winsor's *winsorization*, and I alternatively suggest the (less self-indulgent) name of *r score transformation*. The formula used to *meyerize* vector *x* is:

$$x_meyerized = 2\dfrac{x_i - x_min}{x_max - x_min} - \dfrac{1}{n}\sum_{n=1}^{n} 2\dfrac{x_i - x_min}{x_max - x_min}, x \in \mathbb{R}, x_1,..., x_n$$,


which, unlike other approaches, ensures that it has an approximate mean of 0 and asymptotic range [-1; 1], assuming $$x \tilde \mathcal{N}(\mu,\,\sigma^{2})$$

When applied in the context of linear regression, *meyerizing* both a continuous predictor *x* and outcome *y* enables the coefficient to be interpreted and compared to a correlation coefficient and can be meaningfully interpreted as the following: An increase (decrease) in the predictor from its mean to its highest (lowest) value is associated with an increase (decrease) in the outcome from its mean to its highest (lowest) value by the size of the coefficient.
...
This function uses a novel equation to assist researchers working within the framework of *Bayesian statistics* in specifying their priors with a *t*-distribution. The *Student t*-distribution (Gosset 1908) is highly useful due to its properties of *maximum entropy* (McElreath 2015: 179) and relation to the *Cauchy* and (Gaussian) *Normal* distribution. Where the former is typically used as a relatively skeptical prior, the latter is less skeptical in comparison.

The *t*-distribution is specified using a mean and standard deviation (or variance) parameters, as well as a *degrees of freedom* (df), which controls the width of its 'tails'. Lower values result in 'fatter' tails and higher values with 'slimmer' tails. When df is 1, the *t*-distribution and the *Cauchy*-distribution are equivalent, and when df is infinite, the *t*-distribution and the *normal* distribution are equivalent. This means that df can be used to shape the degree of skepticism in the distribution: The lower the df, the greater the skepticism.

The `meyersDF` function provides an equation that determines df as a function of the number of previously conducted studies and their sample size. This mimicks a qualitive assessment and comes with the advantage of quantifying this assessment and making it reproducible. With few studies and/or a small sample size, the df is rather low to indicate skepticism towards the results from the given field of research. With many studies and/or a large sample size, df is higher and converges to infinity, thus making the distribution equivalent to a prior specified with the Normal distribution.

The function also enables the researcher to specify an arbitrary bias that serves to reflect skepticism towards dubious fields with tradition for using small sample sizes and that systematically fail to replicate, e.g., psychological science (Open Science Collaboration 2015).
