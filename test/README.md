...
$$x_{meyerized} = 2\dfrac{x_i - x_{min}}{x_{max} - x_{min}} - \dfrac{1}{n}\sum_{n=1}^{n} 2\dfrac{x_i - x_{min}}{x_{max} - x_{min}}, x \in \mathbb{R}, x_1,..., x_n,$$

which, unlike other approaches, ensures that it has an approximate mean of 0, and assuming *x* is normally distributed, an asymptotic range [-1; 1].
