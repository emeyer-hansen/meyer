...
<a name="Rescale data with `meyerize`">
   *meyerize*
</a>
<a name="Determine df for *t*-distribution with `meyersDF`">
   *meyersDF*
</a>
<a name="'Exaggeration Level' with `typeMlevel`">
   *typeMlevel*
</a>
<a name="Power Assessment">
   *Power Assessment*
</a>
<a name="'Practical Significance' Level with `pracSigLevel`">
   *pracSigLevel*
</a>
<a name="'Statistical Significance' Level with `statSigLevel`">
   *statSigLevel*
</a>
<a name="Pooled Standard Error with `pooledSE`">
   *pooledSE*
</a>
<a name="Critical Value with `criticalValue`">
   *criticalValue*
</a>
<a name="Compute *p*-values with `pValue`">
   *pValue*
</a>
<a name="Confidence Intervals with `ci`">
   *ci*
</a>
<a name="Changelog">
   Changelog
...
This function computes the *pooled standard error* (Rubin 1987), which is typically used for assessing uncertainty when imputing missing values in some *quantity of interest* (QOI). This metric corresponds to the *total variance* ($V_T$), which accounts for *within variance* ($V_W$), *between variance* ($V_B$), and the number of imputations (\dfrac{V_B}{m}$). `pooledSE` uses the following equations (Heymans & Eekhout 2019):
...
$$100(1-\alpha)\%CI = \bar{\theta} \pm SE\times z, \bar{\theta} \in \mathbb{R}, \{\alpha, z\} \in \mathbb{R^+},$$
...
- [update] 19 Dec 2023: Fix to *pracSigLevel* function, enabling the significance levels of Funder & Ozer (2019) to be used. Added changelog.
