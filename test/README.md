...
This function computes the *pooled standard error* (Rubin 1987), which is typically used for assessing uncertainty in the estimated quantities of interest when imputing missing values. This is due to the equation for the total variance ($V_T$) accounting for both within ($V_W$) and between variance ($V_B$):

$$V_W = \dfrac{1}{m}\sum_{i=1}^{m}SE_i^{2}$$

$$V_B = \dfrac{1}{m-1}\sum_{i=1}^{m}(\theta_i-\overline{\theta})^{2}$$

$$V_T = $$V_W+$$V_B+\dfrac{V_B}{m}$$
...
## Changelog
- [update] 19 Dec 2023: Fix to \code{pracSigLevel} function, enabling the significance levels of Funder & Ozer (2019) to be used. Added changelog.
